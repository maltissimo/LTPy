...
class MathUtils2():
    ...
    def splitimage( nparray2D):
        ...
        """
        center = MathUtils2.centroid(nparray2D)
    ...
    def calc_2D_fwhm(nparray2D):
        HOR_vector, VER_vector = MathUtils2.splitimage(nparray2D)
        """HOR_axis = np.arange(0, len(HOR_vector), 1)
        VER_axis = np.arange(0, len(VER_vector), 1)"""

        HOR_FWHM = MathUtils2.fwhm(HOR_vector)
        VER_FWHM = MathUtils2.fwhm(VER_vector)
